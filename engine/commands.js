...
export function validateRoleSettings(settings, playerCount) {
  ...
  if (playerCount < 5) errs.push("캐릭터는 최소 5명 이상이어야 게임을 시작할 수 있습니다.");
  if (playerCount > 15) errs.push("캐릭터는 최대 15명까지 가능합니다.");

  const maxG = maxGnosiaForPlayerCount(playerCount);
  const g = Number(settings.gnosiaCount ?? 1);
  if (!Number.isFinite(g) || g < 1) errs.push("그노시아 수는 최소 1명 이상이어야 합니다.");
  if (g > maxG) errs.push(`현재 인원(${playerCount}명)에서 그노시아 수는 최대 ${maxG}명까지 가능합니다.`);

  // 선내대기인(2명) 켰는데 인원 부족하면 안됨
  if (settings.enableGuardDuty) {
    if (playerCount < 5) errs.push("선내대기인을 활성화하기엔 인원이 너무 적습니다.");
